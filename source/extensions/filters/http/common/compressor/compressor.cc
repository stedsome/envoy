...
CompressorFilter::chooseEncoding(const Http::ResponseHeaderMap& headers) const {
  ...
  for (const auto& pair : pairs) {
    ...
    if ((pair_q_value > choice_q_value) &&
        (allowed_compressors.count(std::string(pair_encoding)) ||
         pair_encoding == Http::CustomHeaders::get().AcceptEncodingValues.Identity ||
         pair_encoding == Http::CustomHeaders::get().AcceptEncodingValues.Wildcard)) {
      choice = pair;
    }
  }
  ...
  if (choice_encoding == Http::CustomHeaders::get().AcceptEncodingValues.Identity) {
    return std::make_unique<CompressorFilter::EncodingDecision>(
        Http::CustomHeaders::get().AcceptEncodingValues.Identity,
  ...
  if (choice_encoding == Http::CustomHeaders::get().AcceptEncodingValues.Wildcard) {
